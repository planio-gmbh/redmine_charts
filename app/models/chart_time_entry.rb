class ChartTimeEntry < ActiveRecord::Base
  ...
  def self.get_timeline(raw_group, raw_conditions, range)
    ...
    unless range
      row = select("month, week, day").
        joins(joins).
        where(["day > 0 AND chart_time_entries.project_id in (?)",
               conditions['chart_time_entries.project_id']]).
        readonly.
    ...
    rows = joins(joins).select(select).where(conditions).
      readonly.group(grouping).order("1 asc, 6 asc")
  ...
  def self.get_aggregation_for_issue(raw_conditions, range)
    ...
    rows = joins(joins).select(select).where(conditions).
      readonly.group(group).order("1 desc, 2 asc")
  ...
  def self.get_aggregation(raw_group, raw_conditions)
    ...
    rows = joins(joins).select(select).where(conditions).
      readonly.group(group).order("1 desc, 3 asc")
